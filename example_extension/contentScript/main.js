  ...
  void 0 === ytSpeed &&
    ((ytSpeed = {
      ...
      updateVideos: function () {
        for (
          var a = document.querySelectorAll(Elem), b = 0;
          b < a.length;
          ++b
        ) {
          var c = a[b];
          (c.playbackRate = this.playbackRate),
            (c.defaultPlaybackRate = this.playbackRate) /*ensure*/,
            (c.mozPreservesPitch = this.preservesPitch),
            (c.preservesPitch =
              this.preservesPitch); /*add ff101+ compatibility*/
        }
      },
      speedUp: function () {
        (this.playbackRate = meth_upd_p(this.playbackRate, rkc[6])),
          ytSpeed.updateVideos();
      },
      speedDown: function () {
        (this.playbackRate = meth_upd_m(this.playbackRate, rkc[6])),
          ytSpeed.updateVideos();
      },
      reset: function () {
        (this.playbackRate = 1), ytSpeed.updateVideos();
      },
      prompt: function () {
        var a = prompt(
        ...
        a && ((this.playbackRate = ifsemitone(a)), ytSpeed.updateVideos());
      },
    }),
    ytSpeed.init());
  return ytSpeed;
...
function inject_jungle() {
  ...
  ell_SpeedPitchChanger_despaEll_jungle =
  ...
  for (let i = 0; i < ell_SpeedPitchChanger_despaEll_jungle.length; i++) {
    source_SpeedPitchChanger_despaEll_jungle.push(
      context_SpeedPitchChanger_despaEll_jungle.createMediaElementSource(
        ell_SpeedPitchChanger_despaEll_jungle[i],
      ),
    );
  }
  jungle_SpeedPitchChanger_despaEll_jungle = new Jungle(
    context_SpeedPitchChanger_despaEll_jungle,
  );
  jungle_SpeedPitchChanger_despaEll_jungle.output.connect(
    context_SpeedPitchChanger_despaEll_jungle.destination,
  );
...
function inject_jungle_unDOM() {
  context_SpeedPitchChanger_despaEll_jungle = new AudioContext();
  ell_SpeedPitchChanger_despaEll_jungle = SpeedPitchChanger_despaEll_1.concat(
    SpeedPitchChanger_despaEll_2,
  );
  //console.log(ell_SpeedPitchChanger_despaEll_jungle);
  source_SpeedPitchChanger_despaEll_jungle = [];
  for (let i = 0; i < ell_SpeedPitchChanger_despaEll_jungle.length; i++) {
...
function vpup() {
  ...
  let a =
    jungle_SpeedPitchChanger_despaEll_jungle.previousPitchNumber_SpeedPitchChanger_despaEll_jungle;
  jungle_SpeedPitchChanger_despaEll_jungle.setPitchOffset(a + 1, true);
  window.eval(
    "jungle_SpeedPitchChanger_despaEll_jungle.setPitchOffset(jungle_SpeedPitchChanger_despaEll_jungle.previousPitchNumber_SpeedPitchChanger_despaEll_jungle + 1, true);",
  );
...
document.onkeydown = function (evt) {
  if (rkc[5] == 1) {
    ...
    switch (evt.keyCode) {
      case parseInt(rkc[0]):
        notifyBackgroundPage("xpres");
        break;
      case parseInt(rkc[1]):
        notifyBackgroundPage("xpup");
        break;
      case parseInt(rkc[2]):
        notifyBackgroundPage("xpdw");
        break;
      case parseInt(rkc[3]):
        xpdef();
        break;
